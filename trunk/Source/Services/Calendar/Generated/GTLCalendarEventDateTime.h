...
// The date, in the format "yyyy-mm-dd", if this is for an all-day event.
@property (retain) GTLDateTime *date;  // Date only (yyyy-mm-dd)

// The time, as a combined date-time value (optionally with a time zone offset).
@property (retain) GTLDateTime *dateTime;

// The time zone in which the start time is specified. Optional. The default is
